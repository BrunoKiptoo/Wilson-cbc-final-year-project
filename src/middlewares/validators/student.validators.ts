...
export const createStudentValidator = [
    ...
    check('assessments').isArray().withMessage('Assessments should be an array'),
    check('assessments.*.formative').notEmpty().withMessage('Formative assessment is required'),
    check('assessments.*.summative').notEmpty().withMessage('Summative assessment is required'),
    check('competencies.communicationAndCollaboration.score').isInt({ min: 0, max: 100 }).withMessage('Communication and Collaboration score must be between 0 and 100'),
    check('competencies.communicationAndCollaboration.teacher').notEmpty().withMessage('Communication and Collaboration teacher is required'),
    check('competencies.criticalThinkingAndProblemSolving.score').isInt({ min: 0, max: 100 }).withMessage('Critical Thinking and Problem Solving score must be between 0 and 100'),
    check('competencies.criticalThinkingAndProblemSolving.teacher').notEmpty().withMessage('Critical Thinking and Problem Solving teacher is required'),
    check('competencies.imaginationAndCreativity.score').isInt({ min: 0, max: 100 }).withMessage('Imagination and Creativity score must be between 0 and 100'),
    check('competencies.imaginationAndCreativity.teacher').notEmpty().withMessage('Imagination and Creativity teacher is required'),
    check('competencies.citizenship.score').isInt({ min: 0, max: 100 }).withMessage('Citizenship score must be between 0 and 100'),
    check('competencies.citizenship.teacher').notEmpty().withMessage('Citizenship teacher is required'),
    check('competencies.learningToLearn.score').isInt({ min: 0, max: 100 }).withMessage('Learning to Learn score must be between 0 and 100'),
    check('competencies.learningToLearn.teacher').notEmpty().withMessage('Learning to Learn teacher is required'),
    check('competencies.selfEfficacy.score').isInt({ min: 0, max: 100 }).withMessage('Self-Efficacy score must be between 0 and 100'),
    check('competencies.selfEfficacy.teacher').notEmpty().withMessage('Self-Efficacy teacher is required'),
    check('competencies.digitalLiteracy.score').isInt({ min: 0, max: 100 }).withMessage('Digital Literacy score must be between 0 and 100'),
    check('competencies.digitalLiteracy.teacher').notEmpty().withMessage('Digital Literacy teacher is required'),
    check('parents').isArray().withMessage('Parents should be an array'),
    check('parents.*.name').notEmpty().withMessage('Parent name is required'),
    check('parents.*.phoneNumber').notEmpty().withMessage('Parent phone number is required')
];
